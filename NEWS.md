...
- A lab reproduction with the batching above already in place still lost the session at essentially the same wall-clock point as before batching existed: committing more often shortens the transaction, but the underlying connection - and whatever between here and the server is timing it out - stays open the whole time regardless. `episodic_run_cron()` now reconnects between batches (and after the initial setup phase), so the connection itself never accumulates more than roughly one batch's worth of age, not just the transaction on it
