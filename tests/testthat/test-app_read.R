...
test_that("episodic_cluster_object() feeds directly into episodic_interpretation_generate() without error", {
  ...
  expect_true(
    "concentration.high" %in%
      result$fired ||
      "concentration.moderate" %in% result$fired
  )
})
...
test_that("episodic_cluster_object()'s concentration carries a full per-PC breakdown, not just the dominant one", {
  ...
  expect_equal(
    obj$concentration$rows$n[obj$concentration$rows$label == "9711"],
    4
  )
...
test_that("episodic_app_trend() and episodic_app_denominator_series() handle absent data gracefully", {
  ...
  series <- episodic_app_denominator_series(
    env$con,
    "Norovirus",
    episodic_db_cluster_cases(env$con, env$cluster_id)
  )
...
test_that("episodic_farrington_population_vector() is NULL for non-institution levels and levels with no activity data", {
  ...
  expect_null(episodic_farrington_population_vector(
    env$con,
    NA,
    "pathogen_ward",
    weeks
  ...
  expect_null(episodic_farrington_population_vector(
    ...
    "pathogen_ward",
    weeks
  )) # L1: no ward-level activity table
})
...
test_that("episodic_farrington_population_vector() returns a weekly vector for a pathogen_institution stream with activity data", {
  ...
  pop <- episodic_farrington_population_vector(
    env$con,
    env$institution_id,
...
test_that("episodic_app_doubling_time() refuses a doubling time for a flat series", {
  ...
  expect_true(is.na(episodic_app_doubling_time(
    flat,
    asof = as.Date("2025-01-14")
  )))

  declining <- data.frame(
    sample_date = as.character(rep(
      seq(as.Date("2025-01-01"), by = "day", length.out = 6),
      times = c(8, 6, 5, 3, 2, 1)
    ))
  )
  expect_true(is.na(episodic_app_doubling_time(
    declining,
    asof = as.Date("2025-01-06")
  )))
})
...
test_that("episodic_app_doubling_time() reports a plausible doubling time for real growth", {
  ...
  days <- rep(
    seq(as.Date("2025-01-01"), by = "day", length.out = 10),
    times = counts
  )
...
test_that("episodic_app_doubling_time() drops the under-ascertained tail before fitting", {
  ...
  naive <- episodic_app_doubling_time(
    cases,
    incomplete_days = 0L,
    asof = as.Date("2025-01-09")
  )
  trimmed <- episodic_app_doubling_time(
    cases,
    incomplete_days = 3L,
    asof = as.Date("2025-01-09")
  )
