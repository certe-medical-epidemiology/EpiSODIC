...
test_that("episodic_baseline_excluded_windows() returns the window of a confirmed_epidemic cluster and nothing else", {
  ...
  cluster <- DBI::dbGetQuery(
    env$con,
    "SELECT first_day, last_day FROM episodic_cluster WHERE cluster_id = ?",
...
test_that("episodic_baseline_exclude_cases() removes only cases inside an excluded window", {
  ...
  expect_false(any(
    as.Date(filtered$sample_date) >= as.Date("2025-01-03") &
      as.Date(filtered$sample_date) <= as.Date("2025-01-05")
  ))
})
