...
test_that("a single case of a curated rare pathogen fires a detection", {
  ...
  result <- episodic_detect_rare_trigger(con, cases, episodic_db_institutions(con), config)
...
test_that("an empty cases data frame produces no detections", {
  ...
  result <- episodic_detect_rare_trigger(con, empty, episodic_db_institutions(con), config)
  expect_equal(nrow(result), 0)
})
