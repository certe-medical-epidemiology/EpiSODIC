...
pathogen_screen_setup <- function() {
  ...
  pathogen_config <- data.frame(
    ...
    episode_days = c(14, 30),
    incub_min_days = c(1, 1),
    incub_max_days = c(4, 7),
    case_free_days = c(14, 14),
    cooldown_days = c(14, 14),
    rt_applicable = c(1, 0),
  ...
  pathogens <- c(
    rep("Influenza A", length(flu_dates)),
    rep("Campylobacter", length(gi_dates))
  )
  n <- length(dates)
...
test_that("episodic_app_resolve_period() falls back rather than erroring on an unusable custom range", {
  ...
  reversed <- episodic_app_resolve_period(
    "custom",
    from = "2025-01-10",
    to = "2024-01-10",
    asof = asof
...
test_that("episodic_app_resolve_period() honours an explicit custom range", {
  ...
  expect_equal(
    as.integer(period$to - period$from),
    as.integer(period$previous$to - period$previous$from)
  )
})
...
test_that("episodic_app_pathogen_weekly() fades only the weeks still reporting", {
  ...
  complete <- episodic_app_pathogen_weekly(
    cases,
    resolved,
    incomplete_days = 0L,
...
test_that("episodic_app_pathogen_overlay() uses calendar years for a non-seasonal pathogen", {
  ...
  overlay <- episodic_app_pathogen_overlay(
    data.frame(sample_date = dates),
    list(to = as.Date("2024-08-01")),
    seasonal = FALSE
  )
...
test_that("episodic_app_pathogen_summary() compares against the previous period and refuses a ratio to zero", {
  cases <- data.frame(
    sample_date = as.Date(c("2025-01-06", "2025-01-07", "2025-01-08"))
  )
  resolved <- list(
    from = as.Date("2025-01-06"),
    to = as.Date("2025-01-12"),
    previous = list(
      from = as.Date("2024-12-30"),
...
test_that("episodic_app_pathogen_clusters() links the epidemiological view back to the signals", {
  ...
  inside <- episodic_app_pathogen_clusters(
    env$con,
    "Influenza A",
    list(from = as.Date("2025-01-01"), to = as.Date("2025-01-31")),
    lang = "en"
  )
  expect_equal(nrow(inside), 1)
  expect_equal(inside$cluster_id[1], cluster_id)
  expect_equal(
    inside$level_label[1],
    episodic_tr("level.pathogen_region", lang = "en")
  )
  ...
  outside <- episodic_app_pathogen_clusters(
    env$con,
    "Influenza A",
    list(from = as.Date("2023-01-01"), to = as.Date("2023-01-31")),
    lang = "en"
  )
...
test_that("episodic_ui_pathogen_screen() renders without error, empty database included", {
  ...
  for (key in c(
    "pathogen.panel.curve.title",
    "pathogen.panel.overlay.title",
    "pathogen.panel.rt.title",
    "pathogen.panel.clusters.title",
    "pathogen.period.season_current"
  )) {
    expect_true(
      grepl(episodic_tr(key, lang = "en"), html, fixed = TRUE),
      info = key
    )
  }
  ...
  empty_html <- as.character(episodic_ui_pathogen_screen(
    episodic_app_pathogen_screen(con, lang = "en"),
    lang = "en"
  ))
  expect_true(grepl(
    episodic_tr("pathogen.empty", lang = "en"),
    empty_html,
    fixed = TRUE
  ))
})
...
test_that("episodic_mem_threshold_lines() orders thresholds by value and labels each one", {
  ...
  expect_null(episodic_mem_threshold_lines(list(
    pre_epidemic = NA_real_,
...
test_that("the overlay stops the period in progress at the current week", {
  ...
  overlay <- episodic_app_pathogen_overlay(
    data.frame(sample_date = dates),
    list(to = as.Date("2025-01-20")),
    seasonal = FALSE,
    asof = as.Date("2025-01-20")
  )
