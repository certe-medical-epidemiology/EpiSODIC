...
test_that("episodic_closure_criterion_met() fires once case_free_days has elapsed", {
  expect_false(episodic_closure_criterion_met("2025-01-01", NA, case_free_days = 14, today = as.Date("2025-01-10")))
  expect_true(episodic_closure_criterion_met("2025-01-01", NA, case_free_days = 14, today = as.Date("2025-01-15")))
})
...
test_that("episodic_triangle_update() and episodic_triangle_completeness() round-trip", {
  ...
  triangle <- DBI::dbGetQuery(con, "SELECT * FROM episodic_reporting_triangle WHERE stream_id = ?",
...
test_that("episodic_db_denominator_upsert() and episodic_denominator_ingest_run() round-trip", {
  ...
  episodic_db_denominator_upsert(con, pathogen = "Norovirus", sample_date = "2025-01-06",
