...
test_that("input$open_cluster jumps to the Clusters screen on that very cluster", {
  ...
  run_id <- episodic_db_run_start(con, "h", "a")
  first <- episodic_db_cluster_insert(
    con,
    stream_id = stream_id,
    first_day = "2025-01-01",
    last_day = "2025-01-02",
  ...
  second <- episodic_db_cluster_insert(
    con,
    stream_id = stream_id,
...
test_that("a deep link to a closed cluster is not redirected to the top of the rail", {
  ...
  run_id <- episodic_db_run_start(con, "h", "a")
  open_one <- episodic_db_cluster_insert(
    con,
    stream_id = stream_id,
    first_day = "2025-01-01",
    last_day = "2025-01-02",
  ...
  closed <- episodic_db_cluster_insert(
    con,
    stream_id = stream_id,
...
test_that("episodic_app_cluster_viewable() accepts closed clusters but not merged-away ones", {
  ...
  run_id <- episodic_db_run_start(con, "h", "a")
  survivor <- episodic_db_cluster_insert(
    con,
    stream_id = stream_id,
    first_day = "2025-01-01",
    last_day = "2025-01-02",
  ...
  absorbed <- episodic_db_cluster_insert(
    con,
    stream_id = stream_id,
