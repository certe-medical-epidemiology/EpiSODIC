...
same_place_case <- function(source_key,
                            sample_date,
                            institution_id,
...
test_that("n or more cases within k days at the same ward fires a hit", {
  ...
  cases <- rbind(
    same_place_case("K1", "2025-01-01", id),
...
test_that("cases spread beyond k days do not combine into one hit", {
  ...
  cases <- rbind(
    same_place_case("K1", "2025-01-01", id),
    same_place_case("K2", "2025-01-02", id),
    same_place_case("K3", "2025-03-01", id),
    same_place_case("K4", "2025-03-02", id),
...
test_that("non-hospital institutions are scanned at institution level, not ward level", {
  con <- episodic_test_db()
  on.exit(DBI::dbDisconnect(con))
  institutions <- same_place_add_institution(
    con,
    type = "ltc_institution",
    monitored = 0L
  )
  id <- institutions$institution_id[1]
  cases <- rbind(
  ...
  stream <- DBI::dbGetQuery(
    con,
    "SELECT level, ward FROM episodic_stream WHERE stream_id = ?",
