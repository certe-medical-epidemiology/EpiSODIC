...
test_that("concentration: high share (>=0.7), moderate (>=0.5) and diffuse (<0.5) are mutually exclusive", {
  ...
  expect_true("concentration.high" %in% episodic_interpretation_generate(high)$fired)
  expect_true("concentration.moderate" %in% episodic_interpretation_generate(moderate)$fired)
  expect_true("concentration.diffuse" %in% episodic_interpretation_generate(diffuse)$fired)
})
...
test_that("denominator: rising volume with flat positivity is distinguished from rising positivity", {
  ...
  expect_true("denominator.rising_volume_flat_positivity" %in% episodic_interpretation_generate(flat)$fired)
  expect_true("denominator.rising_positivity" %in% episodic_interpretation_generate(rising)$fired)
  expect_true("denominator.stable" %in% episodic_interpretation_generate(stable)$fired)
})
...
test_that("demography slot only fires when a shift is flagged", {
  ...
  expect_true("demography.shifted" %in% episodic_interpretation_generate(shifted)$fired)
  expect_false(any(startsWith(episodic_interpretation_generate(unshifted)$fired, "demography.")))
  expect_false(any(startsWith(episodic_interpretation_generate(no_data)$fired, "demography.")))
})
...
test_that("completeness slot only fires when incomplete_days > 0", {
  ...
  expect_true("completeness.warning" %in% episodic_interpretation_generate(incomplete)$fired)
  expect_false(any(startsWith(episodic_interpretation_generate(complete)$fired, "completeness.")))
})
...
test_that("recommendation always fires exactly one fragment, tiered by priority/detector", {
  ...
  expect_equal(tail(episodic_interpretation_generate(rare)$fired, 1), "recommendation.rare_trigger")
  expect_equal(tail(episodic_interpretation_generate(high)$fired, 1), "recommendation.high_priority")
  expect_equal(tail(episodic_interpretation_generate(moderate)$fired, 1), "recommendation.moderate_priority")
  expect_equal(tail(episodic_interpretation_generate(low)$fired, 1), "recommendation.default")
})
