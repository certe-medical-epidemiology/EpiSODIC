...
test_that("episodic_run_cron() completes successfully end to end on a small synthetic window", {
  ...
  run_id <- episodic_run_cron(path, ingest_source_fn = small_source, run_date = as.Date("2024-08-31"))
...
test_that("running the cron twice over the same window is idempotent on case counts", {
  ...
  episodic_run_cron(path, ingest_source_fn = small_source, run_date = as.Date("2024-06-30"))
  con <- episodic_db_connect(path)
  on.exit(DBI::dbDisconnect(con))
...
test_that("episodic_run_cron() writes denominator rows only when a denominator_source_fn is supplied", {
  ...
  episodic_run_cron(path_without, ingest_source_fn = small_source, run_date = as.Date("2024-06-30"))
  ...
  episodic_run_cron(path_with, ingest_source_fn = small_source, denominator_source_fn = small_denom,
...
test_that("episodic_run_cron() writes institution activity rows only when institution_activity_source_fn is supplied", {
  small_source <- function() {
    episodic_ingest_source_synthetic(
      start_date = as.Date("2024-06-01"), end_date = as.Date("2024-06-30"), seed = 11
    )
  ...
  path_with <- tempfile(fileext = ".sqlite")
  episodic_run_cron(path_with, ingest_source_fn = small_source,
...
test_that("episodic_run_cron() accepts a data frame directly for ingest/denominator/institution_activity_source_fn, not only a function", {
  ...
  path <- tempfile(fileext = ".sqlite")
  episodic_run_cron(path, ingest_source_fn = small_cases, denominator_source_fn = small_denom,
                    run_date = as.Date("2024-06-30"))
  con <- episodic_db_connect(path)
  on.exit(DBI::dbDisconnect(con))
  expect_gt(DBI::dbGetQuery(con, "SELECT COUNT(*) n FROM episodic_case")$n, 0)
  ...
  episodic_run_cron(path2, ingest_source_fn = small_cases, institution_activity_source_fn = small_activity,
                    run_date = as.Date("2024-06-30"))
