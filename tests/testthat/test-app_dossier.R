...
test_that("episodic_ui_dossier() and episodic_ui_assessment_rail() render the fixture cluster without error, in both languages", {
  ...
  expect_s3_class(episodic_ui_dossier(env$con, env$cluster_id, lang = "nl"), "shiny.tag")
  expect_s3_class(episodic_ui_dossier(env$con, env$cluster_id, lang = "en"), "shiny.tag")
  expect_s3_class(episodic_ui_assessment_rail(env$con, env$cluster_id, lang = "nl"), "shiny.tag")
})
...
test_that("episodic_ui_dossier() renders the new M5 panels (Rt, similar clusters, report) with their expected content", {
  ...
  html_anon <- as.character(episodic_ui_dossier(env$con, env$cluster_id, lang = "nl"))
  expect_true(grepl(episodic_tr("panel.rt.title", lang = "nl"), html_anon, fixed = TRUE))
  expect_true(grepl(episodic_tr("panel.similar.title", lang = "nl"), html_anon, fixed = TRUE))
  expect_true(grepl(episodic_tr("panel.report.title", lang = "nl"), html_anon, fixed = TRUE))
  expect_true(grepl(episodic_tr("panel.report.empty", lang = "nl"), html_anon, fixed = TRUE))
  # anonymous viewer gets no render button
  expect_false(grepl("report_render_submit", html_anon, fixed = TRUE))

  html_signed_in <- as.character(episodic_ui_dossier(env$con, env$cluster_id, lang = "nl", current_user = fake_user))
  expect_true(grepl("report_render_submit", html_signed_in, fixed = TRUE))
  expect_true(grepl(episodic_tr("panel.report.render_button", lang = "nl"), html_signed_in, fixed = TRUE))
})
...
test_that("episodic_ui_assessment_rail() renders the classification and mute pickers as coloured buttons, not <select>, with a mute intro paragraph", {
  ...
  expect_true(pos(episodic_tr("verdict.artefact", lang = "nl")) < pos(episodic_tr("verdict.cluster_not_yet", lang = "nl")))
  expect_true(pos(episodic_tr("verdict.cluster_not_yet", lang = "nl")) < pos(episodic_tr("verdict.possible_epidemic", lang = "nl")))
  expect_true(pos(episodic_tr("verdict.possible_epidemic", lang = "nl")) < pos(episodic_tr("verdict.confirmed_epidemic", lang = "nl")))
...
test_that("the status trajectory shows classifications, and labels the pre-assessment period 'unassessed' rather than 'new'", {
  ...
  before <- as.character(episodic_ui_trajectory(obj, timeline_before, lang = "nl"))
  expect_true(grepl(episodic_tr("statusverloop.unassessed", lang = "nl"), before, fixed = TRUE))
  expect_false(grepl(episodic_tr("state.new", lang = "nl"), before, fixed = TRUE))
  ...
  after <- as.character(episodic_ui_trajectory(obj2, timeline_after, lang = "nl"))
  expect_true(grepl(episodic_tr("statusverloop.unassessed", lang = "nl"), after, fixed = TRUE))
  expect_true(grepl(episodic_tr("verdict.possible_epidemic", lang = "nl"), after, fixed = TRUE))
  expect_true(grepl(episodic_tr("verdict.confirmed_epidemic", lang = "nl"), after, fixed = TRUE))
})
...
test_that("episodic_rt_unavailable_reason() distinguishes a missing serial interval from insufficient history", {
  ...
  expect_equal(episodic_rt_unavailable_reason(pc_missing_si), "no_serial_interval")

  pc_ok <- list(rt_applicable = TRUE, si_mean_days = 3, si_sd_days = 1.5)
  expect_true(episodic_rt_unavailable_reason(pc_ok) %in% c("insufficient_history", "epiestim_missing"))

  expect_true(is.na(episodic_rt_unavailable_reason(list(rt_applicable = FALSE))))
