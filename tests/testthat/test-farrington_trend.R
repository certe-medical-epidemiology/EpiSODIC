...
test_that("episode_farrington_trend() backfills up to max_backfill_weeks on a fresh stream", {
  set.seed(1)
  ...
  trend <- episode_farrington_trend(cases, config, run_date = week_starts[n_weeks] + 3,
...
test_that("episode_farrington_trend() only computes one week when trend rows already exist", {
  set.seed(2)
  n_weeks <- 4 * 52 + 10
  week_starts <- seq(as.Date("2020-01-06"), by = "week", length.out = n_weeks)
  counts <- stats::rpois(n_weeks, lambda = 3)
...
test_that("episode_farrington_trend() returns zero rows for insufficient history", {
  cases <- data.frame(sample_date = as.character(seq(as.Date("2025-01-01"), as.Date("2025-03-01"), by = "day")))
  config <- episode_test_config()
  trend <- episode_farrington_trend(cases, config, run_date = as.Date("2025-03-01"))
  expect_equal(nrow(trend), 0)
})

test_that("episode_db_stream_trend_upsert() and episode_db_stream_trend() round-trip and update in place", {
  con <- episode_test_db()
  ...
  episode_db_stream_trend_upsert(con, stream_id, "2025-01-06", n_cases = 3, expected = 2.1, upperbound = 5.0)
  episode_db_stream_trend_upsert(con, stream_id, "2025-01-06", n_cases = 4, expected = 2.1, upperbound = 5.0)

  rows <- episode_db_stream_trend(con, stream_id)
