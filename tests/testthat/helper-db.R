...
app_read_setup <- function() {
  ...
  stream_id <- episodic_db_stream_upsert(
    con, stream_key = episodic_stream_key("pathogen_ward", "Norovirus", institution_id = institution_id, ward = "B4"),
  ...
  episodic_db_run_finish(con, run_id, status = "success", n_streams = 1, n_detections = 1,
