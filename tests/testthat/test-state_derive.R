...
test_that("an event with no verdict yet is In beoordeling (assessing)", {
  expect_equal(
    episodic_derive_state(events_one(verdict = NA_character_)),
    "assessing"
  )
})
...
test_that("explicit closure wins over any classification history", {
  ...
  expect_equal(
    episodic_derive_state(
      events_one(verdict = NA_character_),
      explicitly_closed = TRUE
    ),
    "closed"
  )
...
test_that("a cluster with no assessment history is closed if explicitly_closed, new otherwise", {
  ...
  expect_equal(
    episodic_derive_state(events_none(), explicitly_closed = TRUE),
    "closed"
  )
  expect_equal(
    episodic_derive_state(events_none(), explicitly_closed = FALSE),
    "new"
  )
})
...
test_that("snoozed cluster with a non-terminal verdict is In beoordeling (assessing)", {
  ...
  expect_equal(
    episodic_derive_state(
      events_one(verdict = "cluster_not_yet", snooze_until = future),
      today = Sys.Date()
    ),
    "assessing"
  )
...
test_that("an expired snooze does not suppress the verdict-derived state", {
  ...
  expect_equal(
    episodic_derive_state(
      events_one(verdict = "cluster_not_yet", snooze_until = past),
      today = Sys.Date()
    ),
    "monitoring"
  )
