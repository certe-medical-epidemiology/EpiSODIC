...
reconcile_detect <- function(env, run_id, first_day, last_day, n_cases, detector = "same_place") {
  ...
  episodic_detection_record(env$stream_id, detector, first_day, last_day, n_cases)
}
...
test_that("two clusters growing together merge, the older identity survives, assessment history kept", {
  ...
  clusters_before_merge <- episodic_db_clusters_for_stream(env$con, env$stream_id)
  ...
  episodic_db_app_user_insert(env$con, "tester", "Test User", "tester@example.com", "hash")
  ...
  det_bridge <- episodic_detection_record(env$stream_id, "same_place", "2025-01-01", "2025-01-21", 4)
...
test_that("running the same detection twice creates no new clusters and no duplicate rows", {
  ...
  run2 <- episodic_db_run_start(env$con, "h", "a")
  det2 <- episodic_detection_record(env$stream_id, "same_place", "2025-01-01", "2025-01-05", 3)
  reconcile_run(env, run2, det2)
  n_after_second <- nrow(episodic_db_clusters_for_stream(env$con, env$stream_id))
  ...
  n_cluster_case_rows <- DBI::dbGetQuery(
    env$con, "SELECT COUNT(*) n FROM episodic_cluster_case"
  )$n
...
test_that("a failed run inside the cron transaction leaves no partial state", {
  ...
  n_streams_before <- DBI::dbGetQuery(con, "SELECT COUNT(*) n FROM episodic_stream")$n
  ...
  run_id <- episodic_run_cron(path, ingest_source_fn = bad_source, run_date = as.Date("2024-01-05"))
  ...
  n_streams_after <- DBI::dbGetQuery(con, "SELECT COUNT(*) n FROM episodic_stream")$n
  n_cases_after <- DBI::dbGetQuery(con, "SELECT COUNT(*) n FROM episodic_case")$n
...
test_that("a candidate within cooldown_days of a terminal-verdict cluster is absorbed, not opened as new", {
  ...
  original_id <- episodic_db_clusters_for_stream(env$con, env$stream_id)$cluster_id[1]
