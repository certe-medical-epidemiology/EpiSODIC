...
Every case belongs to several *streams* at once - a ward-level stream,
an institution-level stream, an area, a province, the whole catchment -
each watched independently. Streams are enumerated automatically from
the data every run, so a newly appearing pathogen creates its own
streams without any configuration step:
...
- **`farrington`**
  ...
  the standard statistical aberration detector for streams with enough
  baseline history, run at L1/L2 with a patient-day population offset
  where one exists, so a busy month and a quiet one at identical
...
number of *persistent* clusters an assessor can actually track:

1.  Detections from different detectors whose intervals overlap merge
    into one candidate episode.
2.  Each candidate is matched against currently open clusters in the
    same stream by interval overlap (or proximity within
    `case_free_days`): no match creates a new cluster; one match extends
