...
finish. If your question is not here, [**Getting your data
...
three passes. [**Getting your data
in**](https://certe-medical-epidemiology.github.io/EpiSODIC/articles/data-format.html)
...
- MEM runs at province and region streams (configurable via
  `mem.levels`), for pathogens whose own data show a seasonal pattern
  (or overridden per pathogen via `mem_mode`).
...
`EPISODIC_PATHOGEN_CONFIG` the same way. See [**Tuning detection for
your
catchment**](https://certe-medical-epidemiology.github.io/EpiSODIC/articles/tuning-detection.html)
...
how many systems you combine. See [**Getting your data
in**](https://certe-medical-epidemiology.github.io/EpiSODIC/articles/data-format.html)’s
...
you typed never lingers as their real credential. Pass `role = "viewer"`
instead of the default `"epidemiologist"` for someone who should be able
to see everything, including patient-level detail, but should never
record an assessment themselves. If someone forgets their password or
needs their access removed, that is also a job for whoever administers
the database (there is no self-service reset either) - reach out to them
rather than trying to work around it yourself.

**What is the difference between an “epidemiologist” and a “viewer”
account, and how do I decide who needs which?**
...
re-render reports on demand. `viewer` is read-only: it sees exactly what
a signed-in epidemiologist sees, including patient-level line lists, but
cannot record an assessment. In practice: `epidemiologist` is the right
default for board members whose job is reviewing and classifying
signals; reach for `viewer` for anyone who needs full visibility into
cluster detail without a say in the classification itself (a
data-analyst, a manager kept in the loop, an auditor, a colleague
covering for the board without taking assessment responsibility).
Neither role needs signing in just to see aggregate data - that is
anonymous throughout; signing in as either role is what unlocks
...
- **Hosted somewhere shared**: a server running
  ...
  patient-level detail, and only an `epidemiologist` account can
  classify.
...
Yes - any signed-in epidemiologist can open and classify any cluster;
