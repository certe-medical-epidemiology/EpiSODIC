...
epidemiologist can classify, close and mute clusters, and re-render
reports on demand. `viewer` is read-only: it sees exactly what a
signed-in epidemiologist sees, including patient-level line lists, but
...
cluster detail without a say in the classification itself (a manager
kept in the loop, an auditor, a colleague covering for the board without
taking assessment responsibility). Neither role needs signing in just to
see aggregate data - that is anonymous throughout; signing in as either
role is what unlocks patient-level detail.
