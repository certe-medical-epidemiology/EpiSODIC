...
episode_chart_theme <- function() {
  ...
  ggplot2::theme_minimal(base_family = "") +
    ggplot2::theme(
      ...
      panel.grid.major.y = ggplot2::element_line(colour = pal$rule, linewidth = 0.3),
...
episode_ui_epi_curve_chart <- function(curve, lang = "nl") {
  ...
  ggplot2::ggplot(curve, ggplot2::aes(x = .data$sample_date, y = .data$n_cases)) +
    ggplot2::geom_col(ggplot2::aes(alpha = .data$alpha), fill = pal$petrol, width = 0.7, show.legend = FALSE) +
...
episode_ui_trend_chart <- function(trend, lang = "nl") {
  ...
  ggplot2::ggplot(trend, ggplot2::aes(x = .data$week_start)) +
    ggplot2::geom_ribbon(ggplot2::aes(ymin = 0, ymax = .data$upperbound), fill = pal$band, alpha = 0.35) +
    ...
    ggplot2::scale_colour_manual(values = c(obs = pal$ink, exp = pal$petrolL), labels = legend_labels) +
    episode_chart_theme()
}
...
episode_ui_denominator_chart <- function(series, lang = "nl") {
  ...
  ggplot2::ggplot(series, ggplot2::aes(x = .data$week_start)) +
    ggplot2::geom_col(ggplot2::aes(y = .data$n_tests), fill = pal$lilac, width = 4) +
    ggplot2::geom_line(ggplot2::aes(y = .data$positivity_scaled), colour = pal$pink, linewidth = 0.9) +
    ggplot2::geom_point(ggplot2::aes(y = .data$positivity_scaled), colour = pal$pink, size = 1.6) +
    ...
    ggplot2::theme(axis.title.y = ggplot2::element_text(size = 9, colour = pal$muted),
                   axis.title.y.right = ggplot2::element_text(size = 9, colour = pal$carmine))
}
