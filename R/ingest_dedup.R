...
#' `episode_pathogen_config` (standing brief hard rule 5). Deduplication
#' uses `AMR::get_episode()` when `AMR` is installed. Without it (see
#' `QUESTIONS.md` item 12), a documented fallback is used: isolates for the
#' same patient and organism are sorted by sample date, and a new episode
#' starts whenever the gap since the previous isolate in the group exceeds
#' `episode_days`. This is `AMR::get_episode()`'s own documented algorithm
#' for the default (non-combination) case, so the fallback is an accurate
#' equivalent for that case, not merely an approximation of unknown quality.
#'
...
#'   `mo_code`. A pathogen not present in this table uses the schema default
#'   of 30 days.
...
episode_dedup <- function(raw, pathogen_config) {
  ...
  raw$.episode_days <- pathogen_config$episode_days[
    match(raw$mo_code, pathogen_config$mo_code)
  ]
  raw$.episode_days[is.na(raw$.episode_days)] <- 30

  raw$.sample_date <- as.Date(raw$sample_date)
  groups <- split(seq_len(nrow(raw)), paste(raw$patient_key, raw$mo_code, sep = ""))
...
#'
#' Delegates to `AMR::get_episode()` when available, matching its own
#' documented algorithm otherwise. See `episode_dedup()` for why the
#' fallback is a faithful equivalent rather than a loose approximation.
#'
...
episode_assign_episode_ids <- function(dates_sorted, episode_days) {
  if (requireNamespace("AMR", quietly = TRUE)) {
    return(as.integer(AMR::get_episode(dates_sorted, episode_days = episode_days)))
  }
