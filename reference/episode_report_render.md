...
and registered in `episode_report_render`
(`episode_db_report_render_insert()`) - never overwritten, so what was
...
- user_id:
  ...
  `episode_report_render.user_id`'s own documented meaning).
...
- qmd_path:
  ...
  for `episode_tr(..., lang = d$lang)` usage.
