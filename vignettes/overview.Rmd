...
catchment level), and gives epidemiologists a dossier to assess each one,
with a full audit trail and outbreak reports for clinical colleagues
(e.g. clinical microbiologists) and infection prevention nurses.
...
instance (`r EpiSODIC:::doc_system_file("inst/config/episodic_default_config.yaml")`, `EPISODIC_CONFIG`), so an
...
**Activity** is the audit log: every assessment, closure, mute,
...
various notifications options, and all specific detection algorithm settings.
