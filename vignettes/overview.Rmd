...
House colours (`episodic_palette()`) come from a shipped,
organisation-neutral default, overridable per instance by pointing
`EPISODIC_PALETTE_CONFIG` at a YAML file with an organisation's own
colours - the same mechanism used for detection configuration, a custom
report template, and geographic reference data (see
...
instance (`inst/config/episodic_default_config.yaml`, `EPISODIC_CONFIG`), so an
