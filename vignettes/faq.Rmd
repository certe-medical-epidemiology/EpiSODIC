...
synthetic data, provisions a demo admin account (printed to the
...
`role = "viewer"` instead of the default `"admin"` for someone who
...
<details>
<summary><strong>What is the difference between an "admin" and a "viewer" account, and how do I decide who needs which?</strong></summary>

`admin` is the epidemiologist role: by definition, the person who
assesses clusters and classifies them - the core, shared job of being
on the board. An admin can classify, close and mute clusters, and
re-render reports on demand. `viewer` is read-only: it sees exactly
what a signed-in admin sees, including patient-level line lists, but
cannot record an assessment. In practice: `admin` is the right default
...
- **Hosted somewhere shared**: a server running `episodic_run_app()`
  ...
  patient-level detail, and only an `admin` account can classify.
...
Yes - any signed-in admin can open and classify any cluster; there is no
