...
detectors work (`vignette("detection-reconciliation")`) or how the
configuration overlay system works (`vignette("deployment")`).
...
default. See `vignette("deployment")` for the mechanics.
...
If Farrington fires on modest rises that are within normal variation
for your catchment, widen the statistical bound or raise the
effect-size floor that gates new clusters.

`farrington.alpha` is the significance level for the upper bound.
Lowering it from 0.05 to 0.01 widens the bound, so a run needs a
larger exceedance before it reports one. This is a sensitivity-specificity
...
rare organisms where it should stay low, so per-pathogen overrides
under `same_place.overrides` are usually the better lever:
...
Lowering `default_k_days` requires cases to be more tightly clustered
in time, which reduces noise from background endemic rates.
...
of a parent's cases that a single child must account for to suppress
the parent. Lowering it makes it easier for a child to absorb its
parent. `suppression.parent_diffuse_threshold` (shipped as 0.50) and
...
Too aggressive suppression hides a genuinely independent signal behind
a coincidentally overlapping one. If you find suppressed clusters that
...
`eligibility.min_median_weekly_count`, `eligibility.min_nonzero_week_share`)
determine whether a stream reaches the statistical detectors at all.
Streams that fail still run through `same_place` and `rare_trigger`.
Loosening these lets the model fit on sparser streams, where it may not
be meaningful.
...
says nothing. Lower them for a newer instance, and raise them as
history accrues.
...
which a week counts as part of the trough.
`mem.seasonality_peak_weeks` (shipped as 8) and
`mem.seasonality_min_peak_share` (shipped as 0.40) control the
automatic seasonality test: a pathogen is seasonal under `mem_mode =
'auto'` if at least 40% of its detrended annual cases fall within an
8-week peak window.
...
`vignette("detection-reconciliation")`.
...
`r EpiSODIC:::doc_system_file("inst/config/episodic_default_pathogen_config.csv")`.
...
`same_place` are in the YAML (under `same_place.overrides`), not in
this CSV. The CSV holds per-pathogen *biology*, the YAML holds
per-pathogen *detection rules*.
...
`episodic_validate_detection()` replays generated history through the
full detection pipeline week by week and matches what it found against
seeded outbreaks on case sets. Pass a `config` list (for YAML keys) or
a `pathogen_config` data frame (for CSV columns) with the settings you
want to test:
...
improve sensitivity, reduce false alarms, or affect time-to-detection?
The defaults are deliberately small (one seed, four weeks) so it
returns in seconds; for a proper comparison use more seeds and a longer
window. See `vignette("detection-validation")` for the full framework.
...
detected (time-to-detection). This is not circular for the question
"did this change reduce false alarms", which is exactly what PPV
answers. It cannot measure sensitivity, because an outbreak the
detectors miss never becomes a cluster for anyone to judge.
...
- `vignette("detection-reconciliation")` - how the detectors, reconciliation, suppression, and MEM work
- `vignette("deployment")` - the configuration overlay system and instance setup
- `vignette("detection-validation")` - measuring detection against known truth
- `vignette("environment-variables")` - all `EPISODIC_*` variables including `EPISODIC_PATHOGEN_CONFIG`
