...
`effect_size_floor.min_excess_over_upperbound` default: 3) requires
...
`effect_size_floor.min_ratio_observed_expected` default: 1.5)
...
`suppression.child_dominance_threshold` default: 0.70) is the share
...
parent. `suppression.parent_diffuse_threshold` default: 0.50) and
...
`farrington.b` default: 2) is the number of reference years. The
...
`mem.min_seasons` default: 2) requires two fully-observed prior
seasons before fitting. `mem.min_climatology_years` default: 3)
...
`mem.climatology_smooth_weeks` default: 5) is the smoothing window
...
`mem.seasonality_peak_weeks` default: 8) and
`mem.seasonality_min_peak_share` default: 0.40) control the
...
pathogen you list, non-empty values override the default; empty
cells keep it. Pathogens not in your file keep their row
