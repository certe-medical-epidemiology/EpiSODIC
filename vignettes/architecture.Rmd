...
statistical and rule-based detectors over them, reconciles raised signals
into persistent clusters across repeated runs, and gives a small board of
epidemiologists a dossier to formally assess each one - with a full audit
trail and outbreak reports for clinical colleagues (e.g., clinical
microbiologists) and infection prevention nurses at the end.
...
configuration in it. A running instance - a real organisation's database,
...
# roles, and only the "admin" role can also classify
episodic_run_app(db_path = "/path/to/episodic.sqlite")
```

`episodic_run_cron()` does the expensive work: loading case data, stream
