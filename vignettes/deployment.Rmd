...
returning one is accepted just as well - see `episodic_resolve_data()`.
`vignette("data-format")` documents the exact columns each of the four
data sources (cases, positivity metadata, institution activity,
...
Setting `episodic_check_cases(cases, stop_on_problem = TRUE)` makes the checks
return an error if there is at least one problem, for
use in a script. `episodic_run_cron()` runs that before every run.
...
satisfy the requirements stops with that same message, records it on the run
...
to this repository. `r EpiSODIC:::doc_system_file("inst/config/episodic_default_config.yaml")` ships the documented
...
through a *separate* environment variable (`EPISODIC_STYLE`):
colour and font choice are display concerns, never part of the
...
administers the database runs `episodic_add_user()` at the console.
Signing in unlocks everything the account's role allows, patient-level
detail (the line list) included, for both roles below.
...
by anyone other than the colleagues it is meant for - rather than sitting
behind network controls that already make that impossible - should close
that gap rather than rely on the open default. Set
...
navigation, no status strip, not one row of data. This is enforced on the
**server**, not in the browser - none of those screens is rendered for a
session that has not signed in, so nothing they would contain is ever
serialised into the page. There is correspondingly nothing for a
...
`episodic_add_user()` takes `db_path` (defaulting to `EPISODIC_DB`,
see `vignette("environment-variables")`), not an open connection - it
opens and closes its own via `episodic_db_open()`, so adding an
account is one call at the console. The account is created with
`must_change = TRUE`, so its first real sign-in forces the holder to set
their own password before continuing. Pass `is_admin = TRUE` to give the
new account Settings-screen access as well - you need at least one such
account to manage anything from the dashboard itself; every account
after that can be added either way.

### Watching sign-ins

Both outcomes are recorded. A successful sign-in becomes a `login` event
on the account; a refused one is written to
`episodic_app_login_failure` with the username as typed and which of
three reasons it was - no account with that username, wrong password, or
account deactivated. The sign-in screen itself still says none of that
to whoever is typing: unknown username and wrong password are one
generic refusal to them, so that a stranger cannot use the login form to
find out which usernames exist. The distinction is for the operator, in
...
schema (`r EpiSODIC:::doc_system_file("inst/sql/schema.sql")`) is written once, in SQLite syntax, and
adapted at load time for what differs under MariaDB/MySQL - there is no
...
disagree about how a foreign key written on the column rather than at the
end of the table should be treated - MariaDB creates it, MySQL parses it
and discards it - and EpiSODIC's schema is written in the form SQLite
wants. It is translated for these servers so that the constraints exist
on all three, and the suite counts them on a live server rather than
assuming.
...
`episodic_db_truncate()` - is scoped to EpiSODIC's own tables and cannot
touch a co-tenant's.
...
There is no migration that adds them, because a constraint cannot be added
to a table that already violates it: recreate the database with
...
Anything other than `r EpiSODIC:::doc_foreign_key_count()` means the
schema and the server disagree, and the suite's own live tests check that
same number against a live server on every run.

## Upgrading EpiSODIC

The database records which version of EpiSODIC's schema it was built
with, and `episodic_db_connect()` refuses one it does not recognise
rather than failing later with an unexplained SQL error - or, worse, not
failing and reading a column that has since come to mean something else.
So when you update the package and its schema has moved on, the next run
stops with an error naming the fix:
...
all, since the version table postdates it. `episodic_db_migrate()`
...
exactly like `episodic_add_user()`'s does, so a systemd unit or
...
The dashboard holds its connection open as a WebSocket, not a sequence of
ordinary HTTP requests, so a proxy in front of it needs the `Upgrade`
...
`episodic_report_render()` renders the shipped `r EpiSODIC:::doc_system_file("inst/report/episodic_default_report.qmd")`
...
`trend`, `linelist`, `timeline`, `similar`, `diff`, `small_count_threshold`,
...
to render in any of the dashboard's supported languages. `diff` is
`NULL` for a cluster's first-ever render and otherwise holds what
changed since the previous version -
see `vignette("scheduled-reports")`'s "What changed since last time"
section for what it contains.
...
`vignette("notifications")`; see `vignette("scheduled-reports")` for the
full setup guide, including the cadence and automatic-closure rules and
how to extend a custom template with more patient-level detail than the
shipped one shows.
...
House-style colours and typography are not an optional-dependency concern
at all: the app always ships an organisation-neutral default palette
(`r EpiSODIC:::doc_system_file("inst/config/episodic_default_style.yaml")`), overridable per instance by pointing
`EPISODIC_STYLE` at an organisation's own YAML file with its real
colours and font - no package dependency involved either way. A webfont
named there still needs delivering (self-hosted, or linked from its own
provider), typically via a custom `www/episodic.css`; a system font needs
no such link at all.
...
`vignette("notifications")` for the full setup guide, with step-by-step
instructions for every supported channel.

## See also

- `vignette("data-format")` for the case data requirements this vignette's
  `episodic_run_cron()` calls expect.
- `vignette("environment-variables")` for the full `EPISODIC_*`
  reference table, including every variable named above.
- `vignette("notifications")` for setting up alerts on new clusters
  and run failures.
- `vignette("scheduled-reports")` for emailing a recurring outbreak
  report to colleagues without a dashboard account.
- `vignette("faq")` for hosting choices, account roles, and other
