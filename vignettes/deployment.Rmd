...
```r
Sys.setenv(EPISODIC_DB = "/path/to/episode.sqlite")
...
- **MariaDB/MySQL**: a `mysql://user:password@host:port/dbname` DSN,
  built with `episode_db_dsn_mariadb()` rather than assembled by hand.
...
```r
Sys.setenv(EPISODIC_DB = episode_db_dsn_mariadb(
...
`episode_run_app()`'s `db_path` argument defaults to `EPISODIC_DB`
...
deduplication (`episode_dedup()`) calls `AMR::get_episode()` directly,
and pathogen-name italicisation (`episode_ui_italicise_taxon()`) reads
