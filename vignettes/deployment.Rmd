...
`episodic_validate_cases()` runs the same checks as `episodic_check_cases()`
...
`episodic_provision_user()` at the console.
...
opens and closes its own via `episodic_db_open()`, so provisioning an
...
after that can be provisioned either way.
